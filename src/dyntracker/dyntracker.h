...
struct TrackingNode{
	TrackingNode(PointF p_point, unsigned int p_ts)
		:id(0), position(p_point), age(0.0), events(0), error(0.0), last(p_ts), rate(2000.0), angle(0.0), velocity()
	{

	}

	TrackingNode()
		:id(-1), position(), age(0.0), events(0), error(0.0), last(0), rate(2000.0), angle(0.0), velocity()
	{
